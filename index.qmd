...
*UPDATE: In 2025, The U.S. Environmental Protection Agency removed all data we rely on for mapping underserved and overburdened communities in Tampa Bay, including EJScreen and CEJST. The methods described on this site have now been updated to reflect this change, which requires the user to download archived copies of these datasets from third party sources. TBEP has created archived copies of the national 2022 EJScreen and CEJST 2.0 datasets thanks to the [Harvard Dataverse](https://dataverse.harvard.edu/){target="_blank"}, which are available for direct download from a Dropbox link. You will find that many of the links provided on this site no longer direct to a working webpage. We will be looking for archived versions of the pages and update this site as we find them.*
...
underserved_tract_wgs84 %>%
  ...
  addPolygons(
    ...
    color = "black",
    fillOpacity = 0.7,
    group = "Underserved Census Tract"
  ) %>%
  addLayersControl(
    overlayGroups = "Underserved Census Tract",
...
pal <- colorNumeric(palette = brewer.pal(6, "Reds"), domain = underserved_dbasins_wgs84$pct_under)

underserved_dbasins_wgs84 %>%
  filter(pct_under > 0) %>%
  leaflet() %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    fillColor = ~pal(pct_under),
    weight = 0.5,
    opacity = 1,
    color = "black",
    fillOpacity = 0.7,
    popup = ~paste("Waterbody:", WATERBODY_NAME, "<br>Underserved %:", pct_under)
  ) %>%
  addLegend(
    pal = pal,
    values = ~pct_under,
    title = "Underserved Tracts (% of DB)",
...
tb_equity_wgs84 <- st_transform(tb_equity, crs = 4326)

# Create color palette
pal_burdens <- colorNumeric(palette = brewer.pal(8, "YlOrRd"), domain = tb_equity_wgs84$thresholdEJ_N)
