...
```{r}
mapview(tb_blockgroup, zcol = "P_VULEOPCT", at = seq(0, 100, 20), layer.name = "Supp. Demographic Index (percentile)")
```
...
mapview(underserved_BIL, label = "P_VULEOPCT")
```
...
```{r}
mapview(underserved_other, zcol = "threshold_N", col.regions = brewer.pal(6, "Reds"), layer.name = "No. of Thresholds Met")
```
...
```{r}
mapview(underserved_other, zcol = "underserved", col.regions = list("gray","red"), layer.name = "Underserved Communities (non-BIL)")
```
...
```{r}
mapview(underserved_other, zcol= "funding", col.region = "red", layer.name = "Project Funding") + mapview(underserved_BIL, zcol = "funding", col.region = "blue", layer.name = "Project Funding")
```
...
mapview(tb_dbasins_under, zcol = "pct_under", col.regions = brewer.pal(6, "Reds"), at = seq(0, 100, 20), layer.name = "Underserved Tracts (% of DB)", label = "WATERBODY_NAME")
```
