...
pal_threshold <- colorNumeric(palette = brewer.pal(6, "Reds"), domain = underserved_tract_wgs84$threshold_N)
...
underserved_tract_wgs84 %>% 
  ...
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    fillColor = ~pal_threshold(threshold_N),
    weight = 0.5,
    opacity = 1,
    color = "black",
    fillOpacity = 0.7,
    popup = ~paste("Thresholds Met:", threshold_N)
  ) %>%
  addLegend(
    pal = pal_threshold,
    values = ~threshold_N,
    title = "No. of Thresholds Met",
...
```{r}
# Create color palette for underserved
pal_underserved <- colorFactor(palette = c("gray", "red"), domain = underserved_tract_wgs84$underserved)
...
underserved_tract_wgs84 %>% 
  ...
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    fillColor = ~pal_underserved(underserved),
    weight = 0.5,
    opacity = 1,
    color = "black",
    fillOpacity = 0.7,
    popup = ~paste("Underserved:", underserved)
  ) %>%
  addLegend(
    pal = pal_underserved,
    values = ~underserved,
...
# use bounding box
bb <- st_bbox(tb_tract)
bbox_str <- paste(bb$xmin, bb$ymin, bb$xmax, bb$ymax, sep=",")
url <- paste0(
...
# read only features in the bbox
dbasins <- st_read(url)
```
...
url <- paste0(
  ...
  "Florida_Shoreline_1to12000_Scale_Polygon/MapServer/2/query?",
  "where=1%3D1",
  "&geometry=", bbox_str,
  "&geometryType=esriGeometryEnvelope",
  "&spatialRel=esriSpatialRelIntersects",
...
Create a map showing priority drainage basins based on the presence of underserved communities. Click on the drainage basins to view the name of the water body assessment unit.
...
# Create color palette
pal_dbasins <- colorNumeric(palette = brewer.pal(6, "Reds"), domain = underserved_dbasins_wgs84$pct_under)
...
underserved_dbasins_wgs84 %>% 
  ...
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    fillColor = ~pal_dbasins(pct_under),
    weight = 0.5,
    opacity = 1,
    color = "black",
    fillOpacity = 0.7,
    popup = ~paste("Waterbody:", WATERBODY_NAME, "<br>Underserved %:", round(pct_under, 2))
  ) %>%
  addLegend(
    pal = pal_dbasins,
    values = ~pct_under,
