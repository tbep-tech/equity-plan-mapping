...
```{r}
#| cache: true
urlin <- 'https://www.dropbox.com/scl/fi/514156aicnv7ws5s9na2y/2.0-codebook.zip?rlkey=vmk76qzb6by36zpt6obsol4za&dl=1'
...
dattbcejst_wgs84 <- st_transform(dattbcejst, crs = 4326)
...
underserved_tract_wgs84 %>%
  ...
  addLayersControl(
    overlayGroups = c("Underserved Communities"),
    options = layersControlOptions(collapsed = FALSE)
  )
```
...
```{r}
mined_wgs84 <- st_transform(mined, crs = 4326)
...
# Convert to WGS84 for leaflet
allbfld_wgs84 <- st_transform(allbfld, crs = 4326)
...
urlin <- 'https://www.dropbox.com/scl/fi/1hshluhorw2fgy3shkvjo/EJSCREEN_2022_Supplemental_with_AS_CNMI_GU_VI_Tracts.gdb.zip?rlkey=c1jyluowtutk9vhhhff7wlnfq&dl=1'
