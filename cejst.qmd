...
Load the 'underserved_other' shapefile created from "Mapping underserved communities".

```{r}
#| echo: false
#| eval: true
load(file = 'data/underserved_other.Rdata')

#| eval: true
mapview(underserved_other, layer.name = "Underserved Communities (non-BIL)")
```


We will create representative points out of the underserved EJScreen tracts. We can then attribute the values of the underlying CEJST tracts to each point, and then merge those values into 'underserved_other'. Similar to the thresholds defined for identifying underserved communities, we define "overburdened" communities as those that fall within the 80th percentile (or greater) nationally on at least 1 of the 12 measures of environmental justice outlined above.

```{r}
cejstvalues <- st_point_on_surface(underserved_other) %>%
...
underserved_cejst <- left_join(underserved_other, cejstvalues, by = 'ID')
```
...
underserved_cejst_mines_bfields <- left_join(underserved_cejst_mines, proxbfield, by = 'ID') %>%
  ...
  rowwise() %>%
  select(matches('^threshold|^ID|^under|^over|^funding'))
