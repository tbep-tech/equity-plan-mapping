...
Below, we have provided instructions for replicating our methodology for mapping underserved and overburdened communities in Tampa Bay. This approach goes beyond the EPA's requirements for BIL-funded projects, and thus only applies to underserved communities benefiting from projects funded by other sources. To view instructions for downloading the necessary source data, as well as an overview of the environmental justice metrics we consider, see [Getting environmental justice data](cejst.html).
...
```{r}
mapview(under_over, zcol = "overburdened", col.regions = list("gray","red"), layer.name = "Overburdened")
```
...
tb_equity_other <- under_over %>%
  rowwise() %>%
  select(matches('^threshold|^ID|^under|^over|^EJissues'))


save(tb_equity_other, file = 'data/tb_equity_other.RData')
```
